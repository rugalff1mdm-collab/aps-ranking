...
function effectivePrizePayment(s){
  const p1=String(s.payment_type||'').toLowerCase();
  const p2=String(s.payment_type_2||'').toLowerCase();
  const parceladas=[];
  if(p1==='parcelado') parceladas.push(Number(s.installments||0));
  if(p2==='parcelado') parceladas.push(Number(s.installments_2||0));
  // Quando houver dois parcelamentos, a faixa da premiação usa o maior número de parcelas.
...
app.post('/api/sales',auth,async(req,res)=>{
  try{
    const {client_name,birth_date,sale_date}=req.body||{}; const birthDate=String(birth_date||''); const state=normalizeState(req.body?.state); const sourceId=Number(req.body?.lead_source_id||0);
    const age=calculateAge(birthDate,sale_date);
    const paymentType=String(req.body?.payment_type||'').toLowerCase(); const part1Amount=Number(req.body?.payment_amount_1 ?? req.body?.gross_amount ?? 0);
    const installments=req.body?.installments===undefined||req.body?.installments===''?null:Number(req.body.installments);
    const cardPlatform=String(req.body?.card_platform||'').trim()||null;
    const paymentType2=String(req.body?.payment_type_2||'').toLowerCase(); const part2Amount=Number(req.body?.payment_amount_2||0);
    const installments2=req.body?.installments_2===undefined||req.body?.installments_2===''?null:Number(req.body.installments_2);
    const cardPlatform2=String(req.body?.card_platform_2||'').trim()||null;
    const calc=calculatePaymentPart(part1Amount,paymentType,cardPlatform,installments||1);
    const calc2=part2Amount>0?calculatePaymentPart(part2Amount,paymentType2,cardPlatform2,installments2||1):{rate:0,fee:0,net:0};
    const gross=Number((part1Amount+part2Amount).toFixed(2));
    const value=Number(((calc?.net||0)+(calc2?.net||0)).toFixed(2));
    if(!client_name || !validDate(birthDate) || !Number.isInteger(age) || age<18 || age>120 || !Number.isFinite(value) || value<=0 || !Number.isFinite(gross) || gross<=0 || !validDate(sale_date) || !state || !STATES.includes(state) || !sourceId || !['avista','parcelado'].includes(paymentType)) return res.status(400).json({error:'Cliente, data de nascimento, pagamentos, data, estado, origem e forma de pagamento são obrigatórios'});
    if(!calc) return res.status(400).json({error:'Pagamento 1 inválido: confira plataforma e parcelas'});
    if(paymentType==='parcelado' && (!Number.isInteger(installments)||installments<1||installments>12)) return res.status(400).json({error:'Informe corretamente o número de parcelas (1x a 12x)'});
    if(part2Amount>0 && (!['avista','parcelado'].includes(paymentType2) || !calc2)) return res.status(400).json({error:'Pagamento 2 inválido: confira forma, plataforma e parcelas'});
    if(paymentType2==='parcelado' && (!Number.isInteger(installments2)||installments2<1||installments2>12)) return res.status(400).json({error:'Informe corretamente o número de parcelas (1x a 12x) no pagamento 2'});
    ...
    const result=await dbRun('INSERT INTO sales (consultant_id,client_name,client_age,birth_date,amount,gross_amount,sale_date,state,lead_source_id,payment_type,installments,card_platform,card_fee_rate,card_fee_amount,payment_amount_2,payment_type_2,installments_2,card_platform_2,card_fee_rate_2,card_fee_amount_2) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',[consultantId,String(client_name).trim(),age,birthDate,value,gross,sale_date,state,sourceId,paymentType,installments,cardPlatform,calc?.rate??0,calc?.fee??0,part2Amount||0,part2Amount>0?paymentType2:null,part2Amount>0?installments2:null,part2Amount>0?cardPlatform2:null,calc2?.rate??0,calc2?.fee??0]);
...
app.patch('/api/sales/:id',auth,async(req,res)=>{
  try{
    ...
    const client=String(req.body?.client_name??s.client_name).trim(), birthDate=String(req.body?.birth_date??s.birth_date??''), date=String(req.body?.sale_date??s.sale_date), state=normalizeState(req.body?.state??s.state), sourceId=Number((req.body?.lead_source_id??s.lead_source_id)||0);
    ...
    const paymentType=String(req.body?.payment_type??s.payment_type??'').toLowerCase(); const part1Amount=Number(req.body?.payment_amount_1 ?? req.body?.gross_amount ?? s.gross_amount ?? 0);
    const installments=req.body?.installments===undefined?(s.installments===null||s.installments===undefined?null:Number(s.installments)):(req.body.installments===''||req.body.installments===null?null:Number(req.body.installments));
    const cardPlatform=String(req.body?.card_platform??s.card_platform??'').trim()||null;
    const paymentType2=String(req.body?.payment_type_2??s.payment_type_2??'').toLowerCase(); const part2Amount=Number(req.body?.payment_amount_2 ?? s.payment_amount_2 ?? 0);
    const installments2=req.body?.installments_2===undefined?(s.installments_2===null||s.installments_2===undefined?null:Number(s.installments_2)):(req.body.installments_2===''||req.body.installments_2===null?null:Number(req.body.installments_2));
    const cardPlatform2=String(req.body?.card_platform_2??s.card_platform_2??'').trim()||null;
    const calc=calculatePaymentPart(part1Amount,paymentType,cardPlatform,installments||1);
    const calc2=part2Amount>0?calculatePaymentPart(part2Amount,paymentType2,cardPlatform2,installments2||1):{rate:0,fee:0,net:0};
    const gross=Number((part1Amount+part2Amount).toFixed(2));
    const amount=Number(((calc?.net||0)+(calc2?.net||0)).toFixed(2));
    ...
    await dbRun('UPDATE sales SET consultant_id=?,client_name=?,client_age=?,birth_date=?,amount=?,gross_amount=?,sale_date=?,state=?,lead_source_id=?,payment_type=?,installments=?,card_platform=?,card_fee_rate=?,card_fee_amount=?,payment_amount_2=?,payment_type_2=?,installments_2=?,card_platform_2=?,card_fee_rate_2=?,card_fee_amount_2=? WHERE id=?',[consultantId,client,age,birthDate,amount,gross,date,state,sourceId,paymentType,installments,cardPlatform,calc?.rate??0,calc?.fee??0,part2Amount||0,part2Amount>0?paymentType2:null,part2Amount>0?installments2:null,part2Amount>0?cardPlatform2:null,calc2?.rate??0,calc2?.fee??0,s.id]); res.json({ok:true});
