...
app.get('/api/sales',auth,async(req,res)=>{
  ...
  const rows=await dbAll(`SELECT s.id,s.client_name,s.client_age,s.birth_date,s.amount,s.gross_amount,s.sale_date,s.state,s.lead_source_id,s.payment_type,s.installments,s.card_platform,s.card_fee_rate,s.card_fee_amount,ls.name lead_source_name,u.name consultant_name,u.id consultant_id
...
app.post('/api/sales',auth,async(req,res)=>{
  try{
    const {client_name,birth_date,gross_amount,sale_date}=req.body||{}; const birthDate=String(birth_date||''); const gross=Number(gross_amount); const state=normalizeState(req.body?.state); const sourceId=Number(req.body?.lead_source_id||0);
    const age=calculateAge(birthDate,sale_date);
    const paymentType=String(req.body?.payment_type||'').toLowerCase(); const installments=req.body?.installments===undefined||req.body?.installments===''?null:Number(req.body.installments);
    const cardPlatform=String(req.body?.card_platform||'').trim()||null;
    const calc=(paymentType==='parcelado'||cardPlatform)?calculateCardValues(gross,cardPlatform,installments||1):{rate:0,fee:0,net:Number(gross.toFixed(2))};
    const value=calc?.net;
    ...
    if((paymentType==='parcelado'||cardPlatform) && (!cardPlatform || !calc)) return res.status(400).json({error:'Selecione uma plataforma e uma parcela válida de acordo com a tabela de taxas'});
    ...
    const result=await dbRun('INSERT INTO sales (consultant_id,client_name,client_age,birth_date,amount,gross_amount,sale_date,state,lead_source_id,payment_type,installments,card_platform,card_fee_rate,card_fee_amount) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)',[consultantId,String(client_name).trim(),age,birthDate,value,gross,sale_date,state,sourceId,paymentType,installments,cardPlatform,calc?.rate??0,calc?.fee??0]);
...
app.patch('/api/sales/:id',auth,async(req,res)=>{
  try{
    ...
    const client=String(req.body?.client_name??s.client_name).trim(), birthDate=String(req.body?.birth_date??s.birth_date??''), gross=Number(req.body?.gross_amount??s.gross_amount), date=String(req.body?.sale_date??s.sale_date), state=normalizeState(req.body?.state??s.state), sourceId=Number((req.body?.lead_source_id??s.lead_source_id)||0);
    ...
    const paymentType=String(req.body?.payment_type??s.payment_type??'').toLowerCase(); const installments=req.body?.installments===undefined?(s.installments===null||s.installments===undefined?null:Number(s.installments)):(req.body.installments===''||req.body.installments===null?null:Number(req.body.installments));
    const cardPlatform=String(req.body?.card_platform??s.card_platform??'').trim()||null;
    const hasCard=paymentType==='parcelado'||cardPlatform;
    const calc=hasCard?calculateCardValues(gross,cardPlatform,installments||1):{rate:0,fee:0,net:Number(gross.toFixed(2))};
    const amount=calc?.net;
    ...
    if(hasCard && (!cardPlatform || !calc)) return res.status(400).json({error:'Selecione uma plataforma e uma parcela válida de acordo com a tabela de taxas'});
    await dbRun('UPDATE sales SET consultant_id=?,client_name=?,client_age=?,birth_date=?,amount=?,gross_amount=?,sale_date=?,state=?,lead_source_id=?,payment_type=?,installments=?,card_platform=?,card_fee_rate=?,card_fee_amount=? WHERE id=?',[consultantId,client,age,birthDate,amount,gross,date,state,sourceId,paymentType,installments,cardPlatform,calc?.rate??0,calc?.fee??0,s.id]); res.json({ok:true});
