...
app.post('/api/sales',auth,async(req,res)=>{
  try{
    ...
    if(!client_name||!validDate(birthDate)||!Number.isInteger(age)||age<18||age>120||!validDate(saleDate)||!state||!STATES.includes(state)||!sourceId)return res.status(400).json({error:'Cliente, data de nascimento, pagamentos, data, estado e origem são obrigatórios'});
    const active=parts.filter(p=>p.amount>0); if(!active.length)return res.status(400).json({error:'Informe pelo menos um pagamento'});
    if(active.some(p=>!['avista','parcelado'].includes(p.type)||!p.calc||!validDate(p.date)))return res.status(400).json({error:'Confira forma, data, plataforma e parcelas de cada pagamento'});
    if(active.some(p=>p.type==='parcelado'&&(!Number.isInteger(p.installments)||p.installments<1||p.installments>12)))return res.status(400).json({error:'Informe corretamente as parcelas (1x a 12x)'});
...
app.patch('/api/sales/:id',auth,async(req,res)=>{
  try{
    ...
    const active=parts.filter(p=>p.amount>0); if(!client||!validDate(birthDate)||!Number.isInteger(age)||age<18||age>120||!validDate(date)||!state||!STATES.includes(state)||!sourceId||!active.length)return res.status(400).json({error:'Confira cliente, nascimento, pagamentos, data, estado e origem'});
    if(active.some(p=>!['avista','parcelado'].includes(p.type)||!p.calc||!validDate(p.date)))return res.status(400).json({error:'Confira forma, data, plataforma e parcelas de cada pagamento'});
    if(active.some(p=>p.type==='parcelado'&&(!Number.isInteger(p.installments)||p.installments<1||p.installments>12)))return res.status(400).json({error:'Informe corretamente as parcelas (1x a 12x)'});
