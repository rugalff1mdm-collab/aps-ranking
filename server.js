...
async function dbQuery(sql, params=[]) {
  ...
  if (IS_CF_WORKER) {
    // Hyperdrive maintains the underlying connection pool. Cloudflare's
...
async function init(){
  ...
  await dbQuery(`CREATE TABLE IF NOT EXISTS sales (
    id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  ...
  await dbQuery(`CREATE TABLE IF NOT EXISTS lead_sources (
    id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    name TEXT NOT NULL UNIQUE,
    active INTEGER NOT NULL DEFAULT 1,
    created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP
  )`);
  await dbQuery(`CREATE TABLE IF NOT EXISTS leads (
    id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  ...
  await dbQuery(`ALTER TABLE sales ADD COLUMN IF NOT EXISTS client_age INTEGER`);
  await dbQuery(`ALTER TABLE sales ADD COLUMN IF NOT EXISTS birth_date TEXT`);
  await dbQuery(`ALTER TABLE sales ADD COLUMN IF NOT EXISTS gross_amount REAL`);
  await dbQuery(`ALTER TABLE sales ADD COLUMN IF NOT EXISTS payment_type TEXT`);
  await dbQuery(`ALTER TABLE sales ADD COLUMN IF NOT EXISTS installments INTEGER`);
  await dbQuery(`ALTER TABLE sales ADD COLUMN IF NOT EXISTS card_platform TEXT`);
  await dbQuery(`ALTER TABLE sales ADD COLUMN IF NOT EXISTS card_fee_rate REAL`);
  await dbQuery(`ALTER TABLE sales ADD COLUMN IF NOT EXISTS card_fee_amount REAL`);
  await dbQuery(`ALTER TABLE sales ADD COLUMN IF NOT EXISTS payment_amount_2 REAL`);
  await dbQuery(`ALTER TABLE sales ADD COLUMN IF NOT EXISTS payment_type_2 TEXT`);
  await dbQuery(`ALTER TABLE sales ADD COLUMN IF NOT EXISTS installments_2 INTEGER`);
  await dbQuery(`ALTER TABLE sales ADD COLUMN IF NOT EXISTS card_platform_2 TEXT`);
  await dbQuery(`ALTER TABLE sales ADD COLUMN IF NOT EXISTS card_fee_rate_2 REAL`);
  await dbQuery(`ALTER TABLE sales ADD COLUMN IF NOT EXISTS card_fee_amount_2 REAL`);
  await dbQuery(`ALTER TABLE sales ADD COLUMN IF NOT EXISTS payment_date_1 TEXT`);
  await dbQuery(`ALTER TABLE sales ADD COLUMN IF NOT EXISTS payment_amount_3 REAL`);
  await dbQuery(`ALTER TABLE sales ADD COLUMN IF NOT EXISTS payment_type_3 TEXT`);
  await dbQuery(`ALTER TABLE sales ADD COLUMN IF NOT EXISTS installments_3 INTEGER`);
  await dbQuery(`ALTER TABLE sales ADD COLUMN IF NOT EXISTS card_platform_3 TEXT`);
  await dbQuery(`ALTER TABLE sales ADD COLUMN IF NOT EXISTS card_fee_rate_3 REAL`);
  await dbQuery(`ALTER TABLE sales ADD COLUMN IF NOT EXISTS card_fee_amount_3 REAL`);
  await dbQuery(`ALTER TABLE sales ADD COLUMN IF NOT EXISTS payment_date_2 TEXT`);
  await dbQuery(`ALTER TABLE sales ADD COLUMN IF NOT EXISTS payment_date_3 TEXT`);
  await dbQuery(`UPDATE sales SET payment_date_1=sale_date WHERE payment_date_1 IS NULL OR payment_date_1=''`);
  await dbQuery(`CREATE TABLE IF NOT EXISTS prize_rules (
    id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    name TEXT NOT NULL,
  ...
  await dbQuery(`ALTER TABLE prize_rules ADD COLUMN IF NOT EXISTS min_installments INTEGER`);
  await dbQuery(`CREATE TABLE IF NOT EXISTS prize_losses (
    id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    consultant_id INTEGER NOT NULL REFERENCES users(id),
    rule_id INTEGER,
  ...
  await dbQuery(`CREATE TABLE IF NOT EXISTS prize_adjustments (
    id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    consultant_id INTEGER NOT NULL REFERENCES users(id),
    period_month TEXT NOT NULL,
  ...
  await dbQuery(`ALTER TABLE prize_adjustments ADD COLUMN IF NOT EXISTS target_award_key TEXT`);
  await dbQuery(`CREATE TABLE IF NOT EXISTS supervisor_prize_settings (
    id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    name TEXT NOT NULL UNIQUE,
  ...
  const prizeCount=(await dbQuery('SELECT COUNT(*)::int count FROM prize_rules')).rows[0].count;
  if(!prizeCount){
    const rules=[
    ...
    for(const r of rules) await dbQuery(`INSERT INTO prize_rules(name,category,frequency,payment_type,min_amount,max_installments,min_installments,prize_amount,active,description) VALUES($1,$2,$3,$4,$5,$6,$7,$8,1,$9)`,[r[0],r[1],r[1]==='sale'?'sale':r[1],r[2]||null,r[3],r[4],r[1]==='sale'?(r[4]===6?2:r[4]):null,r[5],r[6]]);
  }
  const supDefaults=[['daily_goal',5715],['daily_prize',50],['weekly_goal',28600],['weekly_prize',100]];
  for(const [n,v] of supDefaults) await dbQuery(`INSERT INTO supervisor_prize_settings(name,value,active) VALUES($1,$2,1) ON CONFLICT(name) DO NOTHING`,[n,v]);
  // Corrige a inversão antiga das metas do supervisor, sem mexer em valores que o ADMIN já tenha personalizado.
  await dbQuery(`UPDATE supervisor_prize_settings SET value=5715 WHERE name='daily_goal' AND value=28600`);
  await dbQuery(`UPDATE supervisor_prize_settings SET value=28600 WHERE name='weekly_goal' AND value=5715`);
  // Padroniza os nomes das premiações de meta diária para não aparecerem como acumulado diário.
  await dbQuery(`UPDATE prize_rules SET name='Meta diária — 1º lugar', description='Meta da equipe de R$ 7.500 no dia; 1º lugar em faturamento bruto' WHERE category='daily_rank' AND prize_amount=50`);
  await dbQuery(`UPDATE prize_rules SET name='Meta diária — 2º lugar', description='Meta da equipe de R$ 7.500 no dia; 2º lugar em faturamento bruto' WHERE category='daily_rank' AND prize_amount=30`);


  for(const name of ['Daniel','Tom','Remalho']){
    await dbQuery('INSERT INTO lead_sources (name,active) VALUES ($1,1) ON CONFLICT (name) DO NOTHING',[name]);
  }
  const email=(envVar('ADMIN_EMAIL') || 'admin@aps.local').trim().toLowerCase();
  const password=envVar('ADMIN_PASSWORD') || '123456';
  const existing=(await dbQuery("SELECT * FROM users WHERE role='admin' ORDER BY id LIMIT 1")).rows[0];
  if(!existing){
    const hash=await bcrypt.hash(password,10);
    await dbQuery('INSERT INTO users (name,email,password_hash,role,goal,active) VALUES ($1,$2,$3,$4,$5,1)',['Administrador',email,hash,'admin',0]);
    ...
    const hash=await bcrypt.hash(password,10);
    await dbQuery('UPDATE users SET email=$1,password_hash=$2,active=1 WHERE id=$3',[email,hash,existing.id]);
...
async function prizeData(month, consultantId=null){
  ...
  for(const c of out){
    ...
    for(const s of ss){
      ...
      if(best && Number(best.prize_amount)>0){
        ...
        c.awards.push({award_key:`sale-${s.id}`,rule_id:best.id,rule_name:outsideInstallment?`${best.name} — 50% fora da faixa`:best.name,amount:awardAmount,date:s.sale_date,reason:detail,estimated:false});
      }
    }
    ...
    for(const [day,ds] of Object.entries(days)){
      ...
      if(best&&Number(best.prize_amount)>0)c.awards.push({award_key:`daily-${best.id}-${day}`,rule_id:best.id,rule_name:best.name,amount:Number(best.prize_amount),date:day,reason:`Acumulado do dia: ${moneyJs(total)}`});
    }
    ...
    for(const [w,ws] of Object.entries(weeks)){
      ...
      if(best&&Number(best.prize_amount)>0)c.awards.push({award_key:`weekly-${best.id}-${w}`,rule_id:best.id,rule_name:best.name,amount:Number(best.prize_amount),date:w,reason:`Acumulado semanal: ${moneyJs(total)}`});
      ...
      if(all&&Number(all.prize_amount)>0)c.awards.push({award_key:`all_days-${all.id}-${w}`,rule_id:all.id,rule_name:all.name,amount:Number(all.prize_amount),date:w,reason:'Venda registrada em todos os dias úteis da semana'});
    }
  }
  ...
  if(dailyRule1||dailyRule2){
    ...
    for(const [day,vals] of Object.entries(byDay)){
      ...
      if(hit[0]&&dailyRule1){const c=byId.get(Number(hit[0][0]));if(c&&Number(dailyRule1.prize_amount)>0)c.awards.push({award_key:`daily_rank-${dailyRule1.id}-${day}`,rule_id:dailyRule1.id,rule_name:dailyRule1.name,amount:Number(dailyRule1.prize_amount),date:day,reason:`1º lugar do dia — equipe fez ${moneyJs(teamTotal)}`})}
      if(hit[1]&&dailyRule2){const c=byId.get(Number(hit[1][0]));if(c&&Number(dailyRule2.prize_amount)>0)c.awards.push({award_key:`daily_rank-${dailyRule2.id}-${day}`,rule_id:dailyRule2.id,rule_name:dailyRule2.name,amount:Number(dailyRule2.prize_amount),date:day,reason:`2º lugar do dia — equipe fez ${moneyJs(teamTotal)}`})}
    }
  }
  const adjustments=await dbAll(`SELECT pa.*,u.name consultant_name FROM prize_adjustments pa JOIN users u ON u.id=pa.consultant_id WHERE pa.period_month=? ORDER BY pa.adjustment_date DESC,pa.id DESC`,[month]);
  for(const a of adjustments){
    const c=byId.get(Number(a.consultant_id)); if(!c) continue;
    const amount=Math.abs(Number(a.amount||0));
    if(a.adjustment_type==='cut' && a.target_award_key){
      const target=c.awards.find(x=>x.award_key===a.target_award_key);
      if(target){
        target.original_amount=Number(target.original_amount ?? target.amount);
        target.cut_amount=Number(target.cut_amount||0)+amount;
        target.amount=Number(Math.max(0,target.original_amount-target.cut_amount).toFixed(2));
        target.cut=true;
        target.cut_reason=a.reason;
        target.cut_date=a.adjustment_date;
        continue;
      }
    }
    const signed=a.adjustment_type==='cut'?-amount:amount;
    c.awards.push({id:`manual-${a.id}`,manual:true,adjustment_type:a.adjustment_type,rule_name:a.adjustment_type==='cut'?'Corte manual':'Premiação manual extra',amount:signed,date:a.adjustment_date,reason:a.reason});
  }
...
app.delete('/api/prize-adjustments/:id',auth,adminOnly,async(req,res)=>{
  try{const r=await dbGet('SELECT id FROM prize_adjustments WHERE id=?',[req.params.id]);if(!r)return res.status(404).json({error:'Lançamento não encontrado'});await dbRun('DELETE FROM prize_adjustments WHERE id=?',[req.params.id]);res.json({ok:true})}
  catch(e){res.status(400).json({error:'Não foi possível excluir o lançamento'})}
});
