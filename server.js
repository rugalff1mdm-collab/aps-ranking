...
async function prizeData(month, consultantId=null){
  ...
  for(const a of adjustments){
    ...
    c.awards.push({id:`manual-${a.id}`,manual:true,adjustment_type:a.adjustment_type,rule_name:a.adjustment_type==='cut'?'Corte manual':'Premiação manual extra',amount,date:a.adjustment_date,reason:a.reason});
  }
...
app.post('/api/prize-adjustments',auth,adminOnly,async(req,res)=>{
  try{
    ...
    const r=await dbRun('INSERT INTO prize_adjustments(consultant_id,period_month,adjustment_type,amount,adjustment_date,reason) VALUES(?,?,?,?,?,?)',[consultantId,month,type,signed,date,reason]);
